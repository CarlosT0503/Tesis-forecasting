"""
Pipeline Naive + Tendencia + Estacionalidad (demanda, univariado) --
COMBINACION NUEVA, NO extraccion de un pipeline completo de legacy.

*** No existe en el notebook legacy un baseline que combine unicamente
tendencia lineal + estacionalidad repetida (residuo=0). La celda 64 (FCNN)
tiene las piezas -- `descomponer_stl`, `forecast_tendencia_lineal`,
`forecast_estacionalidad_repetida` -- pero SIEMPRE acompanadas de un modelo
de residuos (FCNN); nunca se guardo una version sin modelo de residuos. ***

Este modulo reutiliza esas 3 funciones **exactamente iguales** a como estan
en la celda 64, mas la funcion `descomponer_stl` (identica en celdas 60 y
64), y arma: pred_final = tendencia_forecast + estacionalidad_forecast
(residuo asumido 0, sin ningun modelo de residuos). Combinacion nueva
construida a partir de piezas legacy, confirmada explicitamente por el
usuario el 2026-08-09.

Decisiones tomadas para esta combinacion (autorizadas explicitamente):
  - Univariado, sin exogenas.
  - Train/test: ventana FIJA (default 3600h/168h), igual a la que usan
    FCNN/Ensemble -- NO la formula dinamica de Naive/Naive_Trend/AR. Se
    prefirio asi porque STL con period=168 necesita una ventana estable y
    suficientemente larga para una descomposicion razonable; la formula
    dinamica de la celda 45 nunca se penso para eso, mientras que 3600h/
    168h es la ventana con la que esta maquinaria de STL ya se uso en
    legacy (celdas 60 y 64).
  - Metricas: se reutiliza la variante de la celda 64 (mascara `isfinite`,
    con guarda contra entrada vacia -> devuelve dict de NaN) -- misma
    familia que FCNN/Ensemble, de donde viene toda esta maquinaria.
  - Sin tuning de ningun tipo (ni Optuna ni AIC): `trials.csv`/
    `config_usada.csv` quedan vacios, no hay ningun parametro que elegir.
"""
...
TRAIN_LAST_HOURS_DEFAULT = 24 * 30 * 5   # 3600 horas = 5 meses, igual a FCNN/Ensemble
...
# =========================================================
# METRICAS (copia exacta de la celda 64 / fcnn_model.py)
# =========================================================
...
NOMBRE_MODELO = "Naive_Trend_Seasonal"
...
def evaluar_region(region, df, train_hours, forecast_horizon, resultados):
    ...
    try:
        ...
        pred_final = trend_forecast + seasonal_forecast
...
def run(
    ...
    """
    Pipeline Naive+Tendencia+Estacionalidad nuevo (ver docstring del
    modulo). Devuelve (series_df, metricas_df, trials_df, config_usada_df);
    trials_df/config_usada_df siempre vacios (sin tuning).
    """
    if exog_cols:
    ...
    # Checkpoint por region: sin trials/config_usada (sin tuning), ventana
    # fija (forecast_horizon siempre es un entero concreto en este modelo).
    regiones_completas, previos = cargar_checkpoint_regiones(
        output_dir, regions_all, forecast_horizon=forecast_horizon, formato_series="filas",
    )
