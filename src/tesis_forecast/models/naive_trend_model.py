"""
Pipeline Naive + Tendencia (demanda, univariado).

Extraido de la celda 45 del notebook legacy ("Modelos univariados"), bloque
NAIVE TREND dentro de `evaluar_serie`: `forecast_naive_trend(train, horizon)`
ajusta una regresion lineal simple (`np.polyfit(x, train, 1)`) sobre el
train completo y extrapola esa recta para todo el horizonte. Sin
hiperparametros, sin Optuna, sin exogenas.

Modulo separado de `naive_model.py` (no una variante empaquetada junto con
Naive) para que cada uno tenga su propio RUN_NAME/carpeta en
`Pipeline_Resultados/`, igual que se pidio para "LSTM multivariada sobre
residuos" como modelo independiente de `lstm_direct`. En la celda 45
legacy ambos vivian en la misma llamada a `evaluar_serie()` y se guardaban
en el mismo CSV; aqui se separan a nivel de infraestructura (RUN_NAME/
carpeta), no a nivel de formula -- `forecast_naive_trend` es identica.

Mismos cambios mecanicos que `naive_model.py` (ver su docstring para el
detalle): `ejecutar_pipeline()` -> `run(...)`; RESULTS_* -> acumulador
local; 7 regiones sin BCA -> `regions_all` centralizado (8, con BCA);
target unico Demanda (la celda 45 tambien evaluaba Generacion); split
train/test dinamico de la celda 45 como default ("auto"), con la opcion
nueva (no en la celda 45) de fijarlo via `ExperimentConfig`.

Sin exogenas, sin tuning: mismas razones que `naive_model.py`.
"""
...
def _resolver_split(serie, forecast_horizon, train_hours):
    """Ver naive_model.py::_resolver_split -- misma formula, misma logica."""
...
def evaluar_serie(nombre_serie, serie, fechas, forecast_horizon, train_hours, resultados):
    ...
    try:
        ...
        if metricas:
            guardar_metricas(resultados, nombre_serie, "Naive_Trend", False, metricas, "serie_completa")
            guardar_predicciones(resultados, nombre_serie, fechas_test, pred, "Naive_Trend")
...
def run(
    ...
    """
    Equivalente al bloque NAIVE TREND de evaluar_serie() en la celda 45,
    parametrizado. Devuelve (series_df, metricas_df, trials_df,
    config_usada_df); trials_df/config_usada_df siempre vacios.
    """
    if exog_cols:
        raise ValueError("El modelo Naive_Trend es univariado: no acepta exogenas.")

    resultados = _ResultsAccumulator()

    # Checkpoint por region: sin trials/config_usada (Naive_Trend no tunea
    # nada), forecast_horizon dinamico ("auto") salvo override explicito.
    ...
    print("=" * 80)
    print("PIPELINE NAIVE + TENDENCIA DEMANDA")
    print("=" * 80)
