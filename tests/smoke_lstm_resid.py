"""
Smoke test del pipeline LSTM multivariada sobre residuos + Tendencia +
Estacionalidad (models/lstm_resid_model.py).

Corre el pipeline completo (STL + tendencia lineal + estacionalidad
repetida + LSTM sobre el residuo con exogenas al estilo LSTM directa)
contra datos sinteticos minimos, 1 region, 1 trial de Optuna. Requiere
`tensorflow`, `optuna` y `statsmodels` -- NO se pudo ejecutar en el entorno
de desarrollo local (no tiene tensorflow/optuna instalados); pensado para
correr en Colab antes de lanzar el pipeline completo de 8 regiones, igual
que smoke_lstm_direct.py / smoke_ensemble_stl.py.

Uso:
    python tests/smoke_lstm_resid.py
"""
...
from tesis_forecast.models import lstm_resid_model as m

REGION = "BCA"

# WINDOW=168 y STL_PERIOD=168 son fijos -- el train debe ser sustancialmente
# mayor a 168 (y >=168+FORECAST_HORIZON para el lag semanal de las exogenas
# electricas en el tramo futuro) para que STL, las ventanas LSTM y el lag168
# tengan sentido.
TRAIN_HOURS = 600
FORECAST_HORIZON = 48
...
def main():
    tmp_data_dir = tempfile.mkdtemp(prefix="smoke_lstmresid_data_")
    tmp_out_dir = tempfile.mkdtemp(prefix="smoke_lstmresid_out_")
    ...
    try:
        ...
        assert set(trials_df["modelo"]) == {"LSTM_resid"}
        ...
        print("\nSMOKE TEST LSTM_RESID_TREND_SEASONAL: OK")
